...
export function getRussianPostAddressFormatError(text: string): string | null {
  ...
  if (
    !/(?:^|[^\d])(?:ул\.?|улица|пр-кт|просп|переул|б-р|бульв|ш\.|шоссе|наб\.?|пл\.?)(?=[\s,.]|$|\p{L})/iu.test(
      t,
    )
  ) {
  ...
  if (
    !/(?:^|\s)(?:д\.?|дом)\s*\d{1,4}/iu.test(t) &&
    !/[^\d]д[.,\s]+\d{1,4}/iu.test(t)
  ) {
...
export function getCdekPvzTextFormatError(text: string): string | null {
  ...
  if (!/(?:(?:^|[^\d])(?:ул\.?|улица|пр-кт|просп|переул|б-р|бульв|ш\.|шоссе|наб\.?|пл\.?)(?=[\s,.]|$|\p{L}))/iu.test(t)) {
    return "Скопируйте в строку улицу, как в примере: «ул. Ленина» или аналог из карточки ПВЗ на cdek.ru.";
  }
  if (!/(?:(?:^|\s)(?:д\.?|дом)\s*\d{1,4}|,?\s*д[.\s]*\s*\d{1,4})/iu.test(t)) {
